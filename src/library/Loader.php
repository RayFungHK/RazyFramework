  ...
  class Loader
  {
  	...
  	public static function CreateMethod(string $name, callable $callback)
  	{
  		...
  		if (ModuleManager::STATUS_PRELOAD_STAGE !== $manager->getStage()) {
  			new ThrowError('You can only allow creating loader method in preload stage.');
  		}

  		$name = trim($name);
  		if (!preg_match('/^(?![0-9]+)[\w]+$/', $name)) {
  			new ThrowError('Invalid loader name, it allows a-z, A-Z and _ (underscore) only, also the name cannot start from digit.');
  		}
