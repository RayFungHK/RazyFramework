...
namespace RazyFramework
{
  class DOMElement
  {
  	...
  	public const DOMTYPE_DOCUMENT   = 3;
  				...
  				foreach ($elements as $node) {
  					if ('>' === $clip[1] || !$clip[1]) {
  						...
  						foreach ($children as $element) {
  							...
  							if (!$clip[1]) {
  								$collection = array_merge($collection, $element($clip[0]));
  							}
  						}
  					} else {
  	...
  	{
  		if (($tagname && $domElement->getNodeName() === $tagname) || !$tagname) {
  			if ('.' === $classid) {
  				// Class
  	...
  	private function parse(string $html)
  	{
  		...
  		while (preg_match('/(<!--(.*?)-->)|<\s*(!)?\s*([\w-]+)(\s+(?:[^<>\\\\]++|\\\\[<>])+)?>/', $unparsed, $matches, PREG_OFFSET_CAPTURE)) {
  			...
  			if ($matches[0][1] > 0) {
  				...
  				$this->nodes[$textNode->getGuid()] = $textNode;
  			}
  			...
  			if ($matches[1][0]) {
  				...
  				$this->nodes[$commentNode->getGuid()] = $commentNode;
  			} else {
  				...
  				$this->nodes[$elementNode->getGuid()]        = $elementNode;
