...
namespace RazyFramework
{
  return function () {
  	...
  	$profiler->checkpoint('Sample A');
  	$b = str_repeat(' ', 1024 * 1024 * 32);
  	$profiler->checkpoint('Sample B');
  	$getContent = file_get_contents('http://youtube.com');
  	$profiler->checkpoint('Sample C');
  	...
  	$tplManager      = $this->loader->view('profiler')->queue();
  		...
  		foreach ($report as $parameter => $value) {
  			if (is_scalar($value)) {
  				if (preg_match('/^output_buffer|memory_/', $parameter)) {
  					$value = getFilesizeString((float) $value, 2, false, ' ');
  				} else {
  					$value .= ' second';
  				}
